...
def process_videos(videos_path,framerate,output_path,model):
    v_names = os.listdir(videos_path)

    for v in v_names:
        # Creating a VideoCapture object to read the video
        cap = cv2.VideoCapture(os.path.join(videos_path, v))
...
if __name__ == "__main__":
    OUTPUT_PATH = "assets/frames/"
    import os
    from dotenv import load_dotenv
    load_dotenv()
    setup_directories_and_yaml(OUTPUT_PATH)

    process_videos(videos_path="downloads",
                   framerate=60*3,
                   output_path=OUTPUT_PATH,
                   model = YOLO(os.environ["MODEL_PATH"]))
    
    compress_folder(OUTPUT_PATH, OUTPUT_PATH + "/labels.zip")
